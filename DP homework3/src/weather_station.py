...
class WeatherStation(Subject):
    """The WeatherStation is the subject that stores and
    updates weather data, notifying observers when changes occur."""

    def __init__(self) -> None:
        super().__init__()
        self._weather_data = WeatherData(temperature=25.0,
    ...
    def notify(self) -> None:
        ...
        for observer in self._observers:
            observer.update(self._weather_data.temperature,
                            self._weather_data.humidity,
                            self._weather_data.wind_speed)

    def set_weather_data(self, temperature: float,
    ...
    def simulate_weather_change(self) -> None:
        ...
        print(f"\nUpdating weather data: {new_temperature}°C, "
              f"{new_humidity}%, {new_wind_speed} km/h")
